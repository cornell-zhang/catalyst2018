...
But what about predicting handwritten digits in images? As we all know, all files are stored as numbers in electronic devices, including images. For colored images under RGB mode, each pixel will be represented by three numbers ranging from 0 to 255 which corresponding to the colors: red, green, and blue. To simplify our design, images in the training set and the testing set are black and white, that is only "0" or "1" for each pixel. Figure 2 and 3 shows two simple examples of training instances with a resulution of 7x7, which are used in the part 1 of the lab exercise.
...
Google Colab works very similarly to a Google Docs, propogating changes that you may make on the document on your device, to all other devices that have access to the very same document. Since it is based on the Jupyter Notebook, the files have an extension, ".ipynb,". To open these files, you could either double click the file and check the column in the top named "Open with Colaboratory" (Figure 6) or choose the file, right click the mouse and select the item "Open with Colaboratory" (Figure 7).
